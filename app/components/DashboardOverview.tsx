...
export default function DashboardOverview({ summary, findings, carbonHistory, usingMock, mockReason }: { summary: DashboardSummary; findings: Finding[]; carbonHistory: number[]; usingMock: boolean; mockReason?: string }) {
  ...
  const dashboardAgents = [
    ...
    { name: "Energy", state: "success", color: "green", status: `${summary.estimated_carbon_reduction_kg.toLocaleString()} kg` },
  ] as const;
  ...
  return (
    <div className="flex flex-col gap-3 lg:h-[calc(100vh-104px)] lg:overflow-hidden">
      ...
      <section className="grid shrink-0 gap-3 lg:h-[126px] lg:grid-cols-3">
        <DashboardCard title="Carbon emissions" icon={<IconLeaf width={16} height={16} />}><div className="flex items-center gap-3"><div className="shrink-0"><div className="flex items-end gap-1"><strong className="text-[24px] leading-none text-ink">{summary.estimated_carbon_reduction_kg.toLocaleString()}</strong><span className="text-[9px] text-muted">kgCO₂e</span></div><p className="mt-1 text-[9px] text-[var(--color-success)]">This month · estimated</p></div><AnimatedCarbonLine values={carbonHistory} /></div></DashboardCard>
      ...
      <section className="grid shrink-0 gap-3 lg:h-[112px] lg:grid-cols-3">
        ...
        <DashboardCard title="Scan status" icon={<IconCheck width={16} height={16} />}><div className="flex items-center justify-between"><div><strong className="text-[15px] text-ink">Scan complete</strong><p className="mt-1 text-[10px] text-muted">{summary.latest_scan_at ? relativeTime(summary.latest_scan_at) : "—"}</p></div><p className="text-right text-[10px] text-muted"><strong className="block text-[15px] text-ink">{Number(findings[0]?.evidence?.assets_scanned ?? 412)}</strong>assets scanned</p></div><div className="mt-2 h-1 overflow-hidden rounded-full bg-surface"><div className="h-full w-full bg-[var(--color-success)]" /></div></DashboardCard>
